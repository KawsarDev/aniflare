...
function RecentEpisodesPage() {
  const [animes, setAnimes] = useState(null);
  let [page, setPage] = useState(1);

  useEffect(() => {
    axios
      .get(`https://animetize-api.vercel.app/recent-episodes?page=${page}`)
  ...
  return (
    <div>
      <h2 className="text-white text-lg px-3 font-bold my-1">
        Recent Episodes
      </h2>
            ...
            <div>
              <div className="relative">
                ...
                <div className="absolute bottom-0 bg-gradient-to-t from-zinc-950 to-transparent w-full">
                  <span className="bg-cyan-400 text-sm text-black px-1 rounded m-1">
                    {anime?.episodeNumber}
                  </span>
                </div>
              </div>
